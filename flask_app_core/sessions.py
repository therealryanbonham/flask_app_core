...
def setup_sessions(app):
    ...
    if session_type not in VALID_SESSION_TYPE:
        message = 'Invalid session type {}. Valid Options {}'.format(session_type, VALID_SESSION_TYPE)
    ...
    if session_type == 'filesystem':
        ...
        app.config['SESSION_DYNAMODB_TABLE'] = os.environ.get('SESSION_DYNAMODB_TABLE', 'SessionTable')
        app.config['SESSION_DYNAMODB_REGION'] = os.environ.get('SESSION_DYNAMODB_REGION', 'us-east-1')
        app.config['SESSION_DYNAMODB_ENDPOINT_URL'] = os.environ.get('SESSION_DYNAMODB_ENDPOINT_URL', None)
        app.logger.debug('SESSION_DYNAMODB_TABLE: {}'.format(app.config['SESSION_DYNAMODB_TABLE']))
        app.logger.debug('SESSION_DYNAMODB_REGION: {}'.format(app.config['SESSION_DYNAMODB_REGION']))
        app.logger.debug('SESSION_DYNAMODB_ENDPOINT_URL: {}'.format(app.config['SESSION_DYNAMODB_ENDPOINT_URL']))
    ...
    app.logger.debug('Using Security Session Cookie: {}'.format(app.config['SESSION_COOKIE_SECURE']))
    app.logger.debug('Using Session Cookie Domain: {}'.format(
        app.config['SESSION_COOKIE_DOMAIN']))  # override the fl
    app.logger.debug('Using Server Name: {}'.format(app.config['SERVER_NAME']))  # override the fl
    Session(app)
